...
def direct_lookup(byte):
    '''Function to perform a lookup into aes_sbox to get SubBytes'''
    x = byte >> 4  # take most significant bit
    y = byte & 15  # take least significant bit
    return aes_sbox[x][y]


def subbytes(block_list):
    import copy
    '''
    # Test for byte representation:
    # for block in block_list:  # We consider each column separately
    #     print("Lunghezza = ", len(block))
    #     for index, byte in enumerate(block):
    #         print(byte)
    '''
    # print(block_list)  # debug
    working_block_list = copy.deepcopy(block_list)

    for block in working_block_list:  # We consider each column separately
        # In each column, we select a single byte
        for index, byte in enumerate(block):
            # We perform the lookup and store the value
            new_byte = direct_lookup(byte)
            # We swap the value with the new value given by the lookup
            block[index] = new_byte
            # print(new_byte)  # debug

    '''We now have the new block which is made up from the swapped bytes.'''

    # print(block_list)  # debug

    return working_block_list
